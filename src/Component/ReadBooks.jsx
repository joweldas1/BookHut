...
const ReadBooks = () => {
  ...
  return (
    <div>
      {books.map((book, idx) => (
        <div key={idx} className="p-3 md:w-4/5 mx-auto border-2 border-black my-2 rounded-lg border-opacity-25">
          <div className="flex flex-col md:flex-row  md:gap-12 items-center  justify-between  md:px-20 ">
            <div className="text-center"><img className="w-40  bg-slate-200 rounded-lg p-3" src={book.image} alt="" /></div>

            <div className="mt-5 md:mt-0">
              <h3 className="text-2xl font-bold playFair text-[#131313]">{book.bookName}</h3>
              <h4 className="text-lg mt-3 text-[#131313cc] workFair font-semibold">By {book.author}</h4>

              {/* tag----------------------- */}
              <div className="md:flex my-3 md:my-5">
              <div className="flex gap-2 md:items-center text-lg">
                <strong className="">Tags</strong>
                <div className="flex text-lg md:text-lg gap-1 bg-[#23BE0A] bg-opacity-5 text-[#23BE0A] font-semibold workFair py-1 px-3 rounded-xl ">
                  {book.tags.map((t, idx) => (
                    <p className="" key={idx}>
                      #{t}
                    </p>
              ...
              <p className="flex  items-center gap-1  mt-2 text-lg font-semibold"> <TiLocationOutline /> Year of Publishing {book.yearOfPublishing} </p>
